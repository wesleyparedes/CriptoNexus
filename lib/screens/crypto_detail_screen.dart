...
class _CryptoDetailScreenState extends State<CryptoDetailScreen> {
  ...
  void didChangeDependencies() {
    ...
    if (_realController.text.isEmpty && _cryptoController.text.isEmpty) {
      _realController.text =
          NumberFormat.currency(locale: 'pt_BR', symbol: '').format(0.00).trim();
  ...
  Future<void> _fetchChartData() async {
    ...
    final bool cacheValido = cachedData != null &&
        cachedTime != null &&
        (now - cachedTime) < 300000;
    ...
    final uri = Uri.parse(
        "https://api.coingecko.com/api/v3/coins/${widget.crypto.id}/market_chart?vs_currency=brl&days=$_selectedDays");
  ...
  String _formatPriceFull(double price) {
    if (price < 1.0) {
      return NumberFormat.currency(
              locale: 'pt_BR', symbol: 'R\$', decimalDigits: 6)
          .format(price);
    }
    return NumberFormat.currency(
  ...
  void _formatTextField(TextEditingController controller,
      {required bool isBRL}) {
    String cleanValue = controller.text
  ...
  void _onRealChanged(String value) {
    ...
    if (widget.crypto.currentPrice != 0) {
      ...
      _cryptoController.text =
          NumberFormat('#,##0.000000', 'pt_BR').format(result);
    } else {
  ...
  statusBarColor: Colors.black, // preto fixo
  statusBarIconBrightness: Brightness.light, // ícones brancos
    ...
    return Scaffold(
      ...
      appBar: AppBar(
  backgroundColor: Colors.black,
  surfaceTintColor: Colors.black, // evita mudar o tom ao rolar
  scrolledUnderElevation: 0, // impede o brilho roxo ao descer
  elevation: 0,
  centerTitle: true,
        leading: IconButton(
              ...
              Card(
                ...
                child: Padding(
                  ...
                  child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        const Text('Calculadora de Conversão',
                            style: TextStyle(
                                fontSize: 18,
                                fontWeight: FontWeight.bold,
                                color: Colors.white)),
                        ...
                        _buildConversionTextField(
                          ...
                          hintText:
                              'Quantidade de ${widget.crypto.symbol.toUpperCase()}',
                  ...
                  child: Row(children: [
                    ...
                    Expanded(
                      child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            const Text('Você Sabia?',
                                style: TextStyle(
                                    fontSize: 18,
                                    fontWeight: FontWeight.bold,
                                    color: Colors.white)),
                            const SizedBox(height: 4),
                            Text(widget.crypto.curiosity,
                                style: const TextStyle(
  ...
  LineChartData _buildChartData() {
    ...
    final minPrice =
        _chartData.map((e) => e.y).reduce((a, b) => a < b ? a : b);
    final maxPrice =
        _chartData.map((e) => e.y).reduce((a, b) => a > b ? a : b);
        ...
        LineChartBarData(
          ...
          belowBarData: BarAreaData(
            ...
            gradient: LinearGradient(
              colors: [
                priceChangeColor.withOpacity(0.3),
                priceChangeColor.withOpacity(0.0)
              ],
